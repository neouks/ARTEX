...
WITH target AS (
 SELECT a,l.approval_state FROM assets a JOIN task_asset_links l ON l.asset_id=a.id
 WHERE a.id=p_asset_id AND l.task_id=p_task_id
), parents AS (
 SELECT l.approval_state,task_asset_blocked(p_task_id,p.id) AS blocked,
 task_asset_host(p)=task_asset_host(t.a) AS exact
 FROM target t JOIN task_asset_links l ON l.task_id=p_task_id JOIN assets p ON p.id=l.asset_id
 WHERE p.type IN ('root_domain','subdomain','ip')
 AND task_asset_host_within(task_asset_host(t.a),task_asset_host(p))
)
SELECT CASE WHEN task_asset_blocked(p_task_id,p_asset_id) THEN 'blocked'
 ELSE COALESCE((SELECT CASE
 WHEN EXISTS(SELECT 1 FROM parents WHERE blocked OR approval_state='blocked') THEN 'blocked'
 WHEN EXISTS(SELECT 1 FROM parents WHERE approval_state='revoked') THEN 'revoked'
 WHEN (t.a).type NOT IN ('service','endpoint') THEN t.approval_state
 WHEN EXISTS(SELECT 1 FROM parents WHERE exact AND approval_state='pending') THEN 'pending'
 WHEN EXISTS(SELECT 1 FROM parents WHERE exact AND approval_state='approved') THEN 'approved'
 WHEN NOT EXISTS(SELECT 1 FROM parents) OR EXISTS(SELECT 1 FROM parents WHERE approval_state='pending') THEN 'pending'
